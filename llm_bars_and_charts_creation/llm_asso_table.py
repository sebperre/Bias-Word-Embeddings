...
def table_association(file, csv_file_name, pos_class_name="female", neg_class_name="male"):
    ...
    for number in n_list:
        ...
        temp_percent_neg = []
        for es in es_list:
            temp_pos_df = temp_es_df.loc[temp_es_df[f"{pos_class_name}_effect_size"] >= es]
            temp_pos_quantity = len(temp_pos_df.index.tolist())
            temp_pos.append(temp_pos_quantity)

            temp_neg_df = temp_es_df[temp_es_df[f"{pos_class_name}_effect_size"] <= -es]
            temp_neg_quantity = len(temp_neg_df.index.tolist())
            temp_neg.append(temp_neg_quantity)
