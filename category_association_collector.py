...
def category_association_collector(category="gender", pos_name="female", neg_name="male", pos_stimuli = ['female','woman','girl','sister','she','her','hers','daughter'], neg_stimuli = ['male','man','boy','brother','he','him','his','son']):
    #Constants
    STEP = 10000
    PERMUTATIONS = 10000
    ...
    bias_array = np.array([SC_WEAT(embedding_df.loc[word].to_numpy(),pos_embeddings,neg_embeddings,PERMUTATIONS) for word in [str(i) for i in vad_words]])
    bias_df = pd.DataFrame(bias_array,index=vad_words,columns=[f'{pos_name}_effect_size',f'{pos_name}_p_value'])
    bias_df.to_csv(path.join(f"category_association_collector", category, "vad", f'glove_vad_words.csv'))
    print('GloVe VAD')
    ...
    for i in range(10):
        ...
        bias_df.to_csv(path.join("category_association_collector", category, "glove", f'glove_100k_{i}.csv'))
    ...
    pos_embeddings, neg_embeddings = embedding_ft.loc[pos_stimuli].to_numpy(), embedding_ft.loc[neg_stimuli].to_numpy()
    ...
    bias_array = np.array([SC_WEAT(embedding_ft.loc[word].to_numpy(),pos_embeddings,neg_embeddings,PERMUTATIONS) for word in [str(i) for i in vad_words]])
    bias_ft = pd.DataFrame(bias_array,index=vad_words,columns=[f'{pos_name}_effect_size',f'{pos_name}_p_value'])
    bias_ft.to_csv(path.join("category_association_collector", "vad", f'ft_vad_words.csv'))
    print('FT VAD')

    #10k SC-WEATs at a time - 100k most frequent words - FT embedding
    for i in range(10):
        targets = embedding_targets[i*STEP:(i+1)*STEP]
        bias_array = np.array([SC_WEAT(embedding_ft.loc[word].to_numpy(),pos_embeddings,neg_embeddings,PERMUTATIONS) for word in targets])
