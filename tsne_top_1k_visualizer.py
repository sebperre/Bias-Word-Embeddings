...
file_top_100k = "glove_100k.csv"
embedding_filename = "glove.840B.300d.txt"

embedding_100k = pd.read_csv(path.join("top_100k_words", file_top_100k), na_values=None, keep_default_na=False,names=['word','female_effect_size','p_value'],skiprows=1)
top_1k = embedding_100k.head(1000)
target_words = top_1k['word'].tolist()

#Skip row for FT
embedding_df = pd.read_csv(path.join("embeddings", embedding_filename),sep=' ',header=None,index_col=0, na_values=None, keep_default_na=False,quoting=csv.QUOTE_NONE,nrows=1000)
...
tsne_df.to_csv(path.join("tsne",f'tsne_dims_1k.csv'))
print(tsne_df)
...
with open(path.join("tsne",f'tsne_vis_1k.dat'),'w') as writer:
